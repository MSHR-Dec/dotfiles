; Homebrew 版 Neovim 0.12.5 に同梱の lua パーサーが binary_expression/unary_expression の
; "operator" フィールドを持たず、組み込みクエリ(operator: _ @operator)が
; `Invalid field name "operator"` で落ちるため、フィールド参照を使わない形に置き換えてオーバーライドする。
; (それ以外は $VIMRUNTIME/queries/lua/highlights.scm と同一)
